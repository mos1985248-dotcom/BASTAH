    ...
    fetch("/api/subscription/verify-payment", {
      ...
      .then(({ ok, body }) => {
        ...
        if (body.invoiceStatus === "PAID") {
          ...
          setStatus("failed");
          setMessage(
            "لم تكتمل عملية الدفع — يمكنك المحاولة مجدداً من صفحة الباقات"
          );
        }
      })
      .catch(() => {
  ...
  return (
    ...
    >
      <div style={{ maxWidth: 420, textAlign: "center" }}>
        {status === "checking" && (
          <LoadingState label="جاري التحقق من عملية الدفع..." />
        )}
        ...
        {status === "paid" && (
          <>
            <PartyPopper
              ...
              style={{ margin: `0 auto ${t.spacing["3"]}` }}
            />
...
function SubscriptionReturnPage() {
  return (
    <Suspense
      fallback={<LoadingState label="جاري تحميل صفحة الدفع..." />}
    >
