...
// ⚠️ بعض الصفحات (قصتنا، المدونة، العروض) لسه ما بُنيت — الرابط موجود
// حسب خارطة الطريق المتفق عليها وسيُفعَّل أول ما تُبنى الصفحة.
const LINKS = [
  { href: "/", label: "الرئيسية" },
...
export default function NavLinks() {
  ...
  return (
    <nav style={{ display: "flex", gap: t.spacing["6"], overflowX: "auto", paddingBottom: t.spacing["1"] }}>
      {LINKS.map((l) => {
        ...
        return (
          <a
            ...
            style={{
              ...
              color: active ? t.colors.primary[800] : t.colors.text.body,
              background: active ? t.colors.primary[100] : "transparent",
              padding: active ? `8px ${t.spacing["4"]}` : `8px ${t.spacing["1"]}`,
      ...
      <style>{`
        .basita-nav-link:hover { color: ${t.colors.primary[700]} !important; }
      `}</style>
    </nav>
  );
}